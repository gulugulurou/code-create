...
import com.wanan.yuaicodemother.core.saver.CodeFileSaveExecutor;
    ...
    public File generateAndSaveCode(String userMessage, CodeGenTypeEnum codeGenTypeEnum) {
        ...
        return switch (codeGenTypeEnum) {
            case HTML -> {
                ...
                yield CodeFileSaveExecutor.executeSaver(result, CodeGenTypeEnum.HTML);
            }
            case MULTI_FILE -> {
                MultiFileCodeResult result = aiCodeGeneratorService.generateMultiFileCode(userMessage);
                yield CodeFileSaveExecutor.executeSaver(result, CodeGenTypeEnum.MULTI_FILE);
            }
            default -> {
...
//    public File generateAndSaveCode(String userMessage, CodeGenTypeEnum codeGenTypeEnum) {
//        return switch (codeGenTypeEnum) {
//            case HTML -> generaAndSaveHtmlCode(userMessage);
//            case MULTI_FILE -> generaAndSaveMultiCode(userMessage);
...
//    }
    ...
    private File generaAndSaveMultiCode(String userMessage) {
    ...
    private Flux<String> processCodeStream(Flux<String> codeStream, CodeGenTypeEnum codeGenType) {
        ...
        return codeStream.doOnNext(chunk -> {
            ...
            try {
                ...
                // 使用执行器保存代码
                File savedDir = CodeFileSaveExecutor.executeSaver(parsedResult, codeGenType);
                log.info("保存成功，路径为：" + savedDir.getAbsolutePath());
            } catch (Exception e) {
                log.error("保存失败: {}", e.getMessage());
            }
